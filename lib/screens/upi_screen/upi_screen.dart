...
class UpiScreen extends StatefulWidget {
  ...
  const UpiScreen({Key? key, required this.onSuccess}) : super(key: key);
  @override
...
class _UpiScreenState extends State<UpiScreen> {
  ...
  Future<void> _onTap(ApplicationMeta app) async {
    ...
    final errAmount = _validateAmount(_amountController.text);
    setState(() {
      _upiAddrError = errAmount;
    });
    if (errAmount != null) {
      return;
    }
    ...
    final response = await UpiPay.initiateTransaction(
      ...
      transactionNote: 'UPI Payment through Clair',
    );
...
String? _validateAmount(String value) {
  ...
  if (double.tryParse(value)! >= 100000 || double.tryParse(value)! <= 1) {
    return 'Amount should be less than 1,00,000 rupees and more than 1 rupee.';
  }
  return null;
}
